...
function povoarModalDoPokemon(pokemon) {
    ...
    imagemDoPokemon.src = pokemon.sprites.other["official-artwork"].front_default;
    especieDoPokemon.innerHTML = pokemon.species.name;
    alturaDoPokemon.innerHTML = `${(pokemon.height / 10).toFixed(2)} m`;
    pesoDoPokemon.innerHTML = `${(pokemon.weight / 10).toFixed(2)} Kg`;
                    ...
                    Promise.allSettled(
                        evolutions.map((evolucao, index) => {
                            fetch(`https://pokeapi.co/api/v2/pokemon/${evolucao.name}`)
                                .then((response) => response.json())
                                .then((evolucaoJson) => {
                                    evolutions[index].img = evolucaoJson.sprites.other["official-artwork"].front_default;
                                    return evolucaoJson;
                                })
                                .finally(results => {
